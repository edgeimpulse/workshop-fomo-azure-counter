...
/* ***SRH*** Copied in some static stuff from the PnP_Template file because this is some
 *  hackerish BS.
 */
...
/* ***SRH*** Custom functions to send my own telemetry
 *  AKA: More hackerish BS
 */
static int generate_telemetry(uint8_t* payload_buffer, 
                              ...
                              int num_washers)
{
  ...
  // Hardcode telemetry names because I ain't got time to make this nice
  az_span name_screws = AZ_SPAN_FROM_STR("NumberOfScrews");
  az_span name_washers = AZ_SPAN_FROM_STR("NumberOfWashers");
  ...
  // Add screw count telemetry name and value to payload
  rc = az_json_writer_append_property_name(&jw, name_screws);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, num_screws);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding property value to telemetry payload. ");

  // Add washer count telemetry name and value to payload
  rc = az_json_writer_append_property_name(&jw, name_washers);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, num_washers);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding property value to telemetry payload. ");
  ...
  // Make sure we didn't exceed allotted buffer size
  ...
  // Terminate buffer and get length
...
static int send_telemetry(azure_iot_t* azure_iot, int num_screws, int num_washers)
{
  // Check if IoT client object exists
  if (azure_iot == NULL)
  {
    LogError("Azure IoT client connection not established.");
    return RESULT_ERROR;
  }
           ...
           difftime(now, last_telemetry_send_time) >= telemetry_frequency_in_seconds)
  {
    ...
    if (generate_telemetry(data_buffer, DATA_BUFFER_SIZE, &payload_size, num_screws, num_washers) != RESULT_OK)
    {
      LogError("Failed generating telemetry payload.");
      return RESULT_ERROR;
    }

    // Print out payload for testing
...
void setup()
{
  ...
  //***SRH*** Set up second UART for receiving object detection count
  Serial1.begin(9600);
  //***SRH*** End
}
...
void loop()
{
  ...
  //***SRH*** update number of screws and washers
  static int state = 0;
  static int temp_screws = 0;
  static int num_screws = 0;
  static int num_washers = 0;
  static int rx_flag = 0;

  // Simple, hardcoded receiver (once again, I'm lazy)
  ...
  // Send number of screws and washers to IoT Central if connected
  ...
  else
  {
    switch(azure_iot_get_status(&azure_iot))
    {
      case azure_iot_connected:
        ...
        else if (send_telemetry(&azure_iot, num_screws, num_washers) != 0)
        {
          LogError("Failed sending telemetry.");          
        }
